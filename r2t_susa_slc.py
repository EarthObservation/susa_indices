...
def r2t(pth_raster, pth_shp, dir_out, suffix):
    ...
    # images = [a for a in images if 3 < int(basename(a)[4:6]) < 11]
    # images = [images[i:i + 4] for i in range(0, len(images), 4)]

    for image in images:
        # Get name and other parameters
        month = int(basename(image)[-8:-6])
        day = int(basename(image)[-6:-4])
        col_date = f"{month:02}_{day:02}"
        col_labels = ["SIG_ASC_VH", "SIG_ASC_VV", "SIG_DES_VH", "SIG_DES_VV",
                      "COH_ASC_VH", "COH_ASC_VV", "COH_DES_VH", "COH_DES_VV"]
        col_names = [f"{a}_{col_date}" for a in col_labels]
        ...
        tqdm.pandas(desc="nanmean")
        tdf["by_bands"] = tdf.hand_rst.progress_apply(lambda g: np.nanmean(g, axis=(1, 2)))
        ...
        csv_name = "temp_slc_" + col_date + ".csv"
...
if __name__ == "__main__":
    ...
    # ---------
    # Input rasters
    geotiff = "t:\\susa\\slc_stacks\\slc_*.tif"

    # Input shape file
    shapefile = "t:\\susa\\ARSKTRP_ZV_clean\\ZV2017_utm_clean.shp"

    # Output folder and suffix to be added to SHP and CSV files
    output_loc = "t:\\susa\\ZV2017_slc"
    suff = "slc"
